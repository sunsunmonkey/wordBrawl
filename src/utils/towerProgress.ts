...
export const getAscensionRank = (level: number): number =>
  Math.max(1, Math.floor((Math.max(1, level) - 1) / LEVELS_PER_ASCENSION) + 1);

export const getLevelInAscension = (level: number): number =>
  ((Math.max(1, level) - 1) % LEVELS_PER_ASCENSION) + 1;
...
export const levelAscensionLabel = (level: number): string =>
  ascensionLabel(getAscensionRank(level));

export const getTowerAscensionRank = (layer: number): number =>
  Math.max(
    1,
    Math.floor((Math.max(1, layer) - 1) / TOWER_LAYERS_PER_ASCENSION) + 1,
  );

export const getTowerLayerInAscension = (layer: number): number =>
  ((Math.max(1, layer) - 1) % TOWER_LAYERS_PER_ASCENSION) + 1;

export const towerAscensionLabel = (layer: number): string =>
  ascensionLabel(getTowerAscensionRank(layer));
...
const TRAINING_SKILL_POOL: Skill[] = [
  {
    name: "训练技·疾突",
    description: "训练场磨出的快速突袭，造成稳定伤害。",
    type: "attack",
    damageMultiplier: 1.35,
  },
  {
    name: "训练技·稳守",
    description: "短时间稳住架势，提升防御。",
    type: "buff",
    damageMultiplier: 0,
    buffPercent: 22,
    buffTurns: 2,
...
const buildTrainingSkill = (level: number, index: number): Skill => {
  const template =
    TRAINING_SKILL_POOL[(level + index) % TRAINING_SKILL_POOL.length];
  return {
    ...template,
...
export interface ApplyTrainingXpResult {
  /** 已经把等级成长结果应用到 character 上；进化阶段保持不变，交给塔战结算演出补触发。 */
  character: RosterCharacter;
  events: LevelUpEvent[];
  triggeredSkillUnlock: boolean;
  learnedSkills: Skill[];
...
export const applyXp = (
  ...
  if (missedEvolution) {
    ...
    events.push({
      newLevel: level,
      evolutionStage: missedEvolution,
      ascensionRank: getAscensionRank(level),
    });
  }
  ...
  while (true) {
    ...
    const localLevel = getLevelInAscension(level);
    const unlockSkill = (SKILL_UNLOCK_LEVELS as readonly number[]).includes(
      localLevel,
    );
    ...
    if (!evolveTrigger) {
      const missingEvolution = getNextMissingEvolutionStage(
        evolutionStage,
        level,
      );
      if (missingEvolution) {
...
/** 小游戏训练用：只处理 XP、等级和基础属性成长，不直接推进形态阶段。 */
export const applyTrainingXp = (
  character: RosterCharacter,
  xpDelta: number,
): ApplyTrainingXpResult => {
  let xp = character.xp + Math.max(0, xpDelta);
  let level = character.level;
  let working: CharacterData = character;
  const events: LevelUpEvent[] = [];
  ...
  while (true) {
    ...
    if (unlockSkill) {
      unlockedSkill = true;
      if (character.skills.length + learnedSkills.length < 8) {
        learnedSkills.push(buildTrainingSkill(level, learnedSkills.length));
      }
    }
    events.push({
      newLevel: level,
  ...
  const skills =
    learnedSkills.length > 0
      ? [...character.skills, ...learnedSkills]
      : character.skills;
...
export const xpProgress = (
  level: number,
  xp: number,
): { current: number; need: number; ratio: number } => {
  const need = xpToNextLevel(level);
...
export const evolutionLabel = (stage: EvolutionStage): string =>
  EVOLUTION_LABELS[stage] ?? EVOLUTION_LABELS[0];
...
export const isSkillUnlockLevel = (level: number): boolean =>
  (SKILL_UNLOCK_LEVELS as readonly number[]).includes(
    getLevelInAscension(level),
  );

/** 当前角色还能再加几个技能槽（最多 8 个，含大招） */
export const remainingSkillSlots = (char: { skills: Skill[] }): number =>
  Math.max(0, 8 - char.skills.length);
...
export const buildLocalEvolution = (
  char: CharacterData,
  ...
  const newUltimate = ultimate
    ? buildEvolvedUltimate(ultimate, stage)
    : undefined;
...
const buildEvolvedUltimate = (
  ...
  return {
    ...
    damageMultiplier: Math.max(
      ultimate.damageMultiplier || 0,
      EVOLUTION_ULT_MULTIPLIER[stage],
    ),
    type: "ultimate",
    isUltimate: true,
    ultimateType: ultimate.ultimateType,
    imageUrl: ultimate.imageUrl,
  };
};

const stripEvolutionSuffix = (name: string): string =>
  name.replace(/·(觉醒|淬炼|升华|破界|神临|超凡)$/u, "").slice(0, 18) ||
  "终极一击";
