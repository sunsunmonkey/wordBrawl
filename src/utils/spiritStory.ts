...
const STORY_CONTEXT_MESSAGE_LIMIT = 10;
const STORY_CONTEXT_MESSAGE_LENGTH = 320;

const sanitizeMessages = (messages: SpiritStoryMessage[]) =>
  messages.slice(-STORY_CONTEXT_MESSAGE_LIMIT).map((message) => ({
    role: message.role,
    speakerRosterId: message.speakerRosterId,
    speakerName: message.speakerName,
    content: message.content.slice(0, STORY_CONTEXT_MESSAGE_LENGTH),
  }));
