...
import { Zap, Shield, Activity, Flame } from "lucide-react";
import type { CharacterData, BattleEvent, Skill } from "../store/useGameStore";
import { ULTIMATE_THRESHOLD } from "../utils/battleEngine";
import { getUltimateTypeById } from "../data/ultimateTypes";

// ⚠️ 本文件为九层塔战斗特效层的共享抽取版，Tower(BattleScreen) 与社交对战(SocialBattleScreen) 共用同一套演出。
// 导出：CharacterCard(大立绘卡)、UltimateOverlayView(大招全屏过场)、useBattleFx(日志驱动特效 hook)。

export type { CharacterData, Skill };

export interface PopupDamage {
  id: string;
  side: "left" | "right";
  value: number;
  isCrit: boolean;
...
/** 大招全屏特效组件 */
export const UltimateOverlayView: React.FC<{ overlay: UltimateOverlay }> = ({
  overlay,
}) => {
...
/**
 * 战斗特效状态与播放逻辑。Tower（单机）与社交对战（联机）共用。
 * playLogEffects(log) 按日志内容播放：大招三段式全屏过场、伤害/治疗飘字、
  ...
  const playLogEffects = useCallback(async (log: BattleEvent) => {
    ...
    if (log.damage && log.attacker !== "system") {
      ...
      setTimeout(() => {
        setPopups((prev) => ({
          ...prev,
          [targetSide]: prev[targetSide].filter((pp) => pp.id !== popupId),
        }));
      }, 1300);
    }
    ...
    if (log.heal && log.attacker !== "system") {
      ...
      setTimeout(() => {
        setPopups((prev) => ({
          ...prev,
          [healerSide]: prev[healerSide].filter((pp) => pp.id !== popupId),
        }));
      }, 1300);
    }
  }, []);
